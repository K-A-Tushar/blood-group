...
trait HasBloodGroup
{
    public function bloodGroupRelation(): BelongsTo
    {
    ...
    protected function bloodGroup(): Attribute
    {
        return Attribute::make(
            get: fn() => $this->bloodGroupRelation?->name,
            set: function ($value) {
    ...
    {
        $this->bloodGroupRelation()->dissociate();
        $this->save();
    }
    ...
    public function scopeWhereBloodGroup(Builder $query, string|int|BloodGroup|null $bloodGroup): Builder
    {
        return match (true) {
            ...
            is_string($bloodGroup)           => $query->whereHas('bloodGroupRelation', fn($q) => $q->where('name', strtoupper($bloodGroup))),
            default                          => $query,
        };
    }
    public function scopeWhereBloodGroupName(Builder $query, string $name): Builder
    {
        return $query->whereHas('bloodGroupRelation', function ($q) use ($name) {
